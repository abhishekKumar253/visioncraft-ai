...
import { useCraftWorkbench } from "@/context/CraftWorkbenchContext";
import { GenerateButton, StylePresetCard } from "./workbench-ui";

export function CraftControlsPanel() {
  const {
    error,
    file,
    inputId,
      ...
      <div className="studio-panel-inset mt-7 rounded-[1.8rem] border p-5 sm:p-6">
        <div className="flex items-center justify-between gap-4">
          ...
          {file ? (
            <Button
              variant="outline"
              size="sm"
              className="Craft-pill gap-2 rounded-full px-3.5 py-1.5 text-xs"
              asChild
            >
        ...
        <div className="mt-5 flex flex-col gap-4 rounded-[1.45rem] border border-border/35 bg-background/22 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-4">
            <Button
              className="Craft-primary-action rounded-full p-5 text-base font-semibold"
              asChild
            >
              <label htmlFor={inputId} className="cursor-pointer">
