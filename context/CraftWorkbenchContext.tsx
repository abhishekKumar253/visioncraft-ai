...
type CraftWorkbenchContextValue = {
  error: string | null;
  file: File | null;
...
const CraftWorkbenchContext =
  createContext<CraftWorkbenchContextValue | null>(null);

const uploadInputId = "craft-image-upload";
...
export function CraftWorkbenchProvider({
  children,
  clerkUserId,
  initialHistory,
  initialQuota,
}: PropsWithChildren<CraftWorkbenchProps>) {
  const value = useCraftWorkbenchValue({
    clerkUserId,
    initialHistory,
    initialQuota,
  });
  ...
  return (
    <CraftWorkbenchContext.Provider value={value}>
      {children}
    </CraftWorkbenchContext.Provider>
  );
}

export function useCraftWorkbench() {
  const value = useContext(CraftWorkbenchContext);

  if (!value) {
    throw new Error(
      "useCraftWorkbench must be used within CraftWorkbenchProvider."
    );
  }
...
function useCraftWorkbenchValue({
  clerkUserId,
  initialHistory,
  initialQuota,
}: CraftWorkbenchProps): CraftWorkbenchContextValue {
  ...
  async function handleSubmit(event: SubmitEvent<HTMLFormElement>) {
    ...
    try {
      ...
      Sentry.logger.info("craft.generation_succeeded", {
